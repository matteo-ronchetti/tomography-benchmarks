...
def parallel(size=256, batch_size=32, num_angles=-1, det_count=-1, op="forward"):
...
def fanbeam(size=256, batch_size=32, num_angles=-1, det_count=-1, s_dist=-1, d_dist=-1, op="forward"):
    num_angles = num_angles if num_angles > 0 else size
    det_count = det_count if det_count > 0 else size
